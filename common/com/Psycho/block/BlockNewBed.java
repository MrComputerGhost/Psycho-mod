...
public class BlockNewBed extends BlockDirectional
{
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9)
    {
        ...
        else
        {
            ...
            if (par1World.provider.canRespawnHere() && par1World.getBiomeGenForCoords(par2, par4) != BiomeGenBase.hell)
            {
                ...
                ItemStack helmitem = Minecraft.getMinecraft().thePlayer.inventory.armorInventory[3]; 
            	
                if ((enumstatus == EnumStatus.OK) && ((helmitem == null) || (helmitem.itemID != IBAManager.Nightcap.itemID)))
                {
                	setBedOccupied(par1World, par2, par3, par4, true);
                	par5EntityPlayer.worldObj.setWorldTime(0);
                	par5EntityPlayer.sleepInBedAt(par2, par3, par4);
                	par5EntityPlayer.addChatMessage("§1§oHmm... no dreams tonight. Why not try wearing a Nightcap?");
                    return true;
                }
                if ((enumstatus == EnumStatus.OK) && (helmitem.itemID == IBAManager.Nightcap.itemID && helmitem != null))
            	{
            		par5EntityPlayer.travelToDimension(-1);
            		par5EntityPlayer.worldObj.setWorldTime(0);
            		par1World.getMoonPhase();
            		return true;
            	}
                else
                {
